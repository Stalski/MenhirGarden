...
public class ImageThreadLoader {
	...
	public static Bitmap readBitmapFromNetwork( URL url ) {
		...
		try {
			...
			Log.d("ImageThreadLoader", "Loaded remote image: " + url.getPath());
		} catch (MalformedURLException e) {
			Log.e("ImageThreadLoader", "Bad ad URL", e);
		} catch (IOException e) {
			Log.e("ImageThreadLoader", "Could not get remote image for " + url.getPath(), e);
		} finally {
			try {
				if( is != null )
					is.close();
				...
				Log.w("ImageThreadLoader", "Error closing stream.");
			}
		}
		return bmp;
	}
